...
Class Jcrop_ajax extends CI_Controller {
    ...
    function load_uploader(){
	 ...
	 function load_jcrop(){
		...
		$resized_filepath = APPPATH.'../resources/images/'.$controlador.'/'.$controlador_id.'/'.$medida.'_'.$nombre.'_redimensionada.'.$ext;
		$original_filepath = APPPATH.'../resources/images/'.$controlador.'/'.$controlador_id.'/'.$medida.'_'.$nombre.'.'.$ext;
    ...
    function upload_photo(){
		...
		///////////////////////////////////
				...
				$path = APPPATH.'../resources/images/'.$controlador.'/'.$controlador_id.'/';
		        ...
		        if( $siempre_jpg==1 AND !in_array( strtolower($ext), array('jpg','jpeg')) ){
		        	$ext = 'jpg';
		        	$new_original_filepath = $path.$randname.'.'.$ext;
		        	imagejpeg( imagecreatefromstring(file_get_contents($original_filepath)), $new_original_filepath, 100);
	...
	function delete_photo(){
		...
		if( $foto_id>0 ){
            ...
            if( $query->num_rows()>0 ){
				...
				$foto = $query->row();
				$path = APPPATH.'../resources/images/'.$controlador.'/'.$controlador_id.'/'.$foto->filename.'*';
		        $files = glob($path);
		        if(is_array($files)){
